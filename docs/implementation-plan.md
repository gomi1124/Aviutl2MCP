...
- WP6進捗: W6.1～W6.2完了。`object.create`・`object.createMedia`・`object.createAlias`・`object.move`・`object.delete`・`object.setName`のpreflight、単一edit callback、dry-run無変更、revision競合、Locator再解決、source/destination layer lock・衝突・effect/media検証、移動時length保持、削除前snapshot、no-op revision不変、複数Object alias事後列挙、at-most-once再送、postcondition失敗時の`partial_operation`とUndo推奨を実装。6操作はApplication edit serviceとMCP toolへ先行公開し、instance/Locator選択、入力上限、offline error、16 tool/5 resource列挙をNative 23 checkと.NET 112 test（実`.aux2` handler相互運用を含む）で検証済み
