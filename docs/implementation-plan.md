...
- WP6進捗: W6.1完了。`object.create`・`object.createMedia`・`object.createAlias`のpreflight、単一edit callback、dry-run無変更、revision競合、layer lock・衝突・effect/media検証、複数Object alias事後列挙、at-most-once再送、postcondition失敗時の`partial_operation`とUndo推奨をNative 22 checkと.NET 105 test（実`.aux2` handler相互運用を含む）で検証済み
