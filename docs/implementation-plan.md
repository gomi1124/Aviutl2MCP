...
- WP6進捗: W6.1～W6.4完了。`object.create`・`object.createMedia`・`object.createAlias`・`object.move`・`object.delete`・`object.setName`・`effect.setItem`・`effect.setState`・`layer.set`・`view.setCursor`のpreflight、単一edit callback、dry-run無変更、content/view revision分離、revision競合、Locator/effect occurrence再解決、source/destination layer lock・衝突・effect/media/codec検証、SDK補正値再取得、移動時length保持、削除前snapshot、no-op revision不変、複数Object alias事後列挙、at-most-once再送、postcondition失敗時の`partial_operation`とUndo推奨を実装。10操作をApplication edit serviceとMCP toolへ公開し、instance/Locator選択、入力上限、offline error、20 tool/5 resource列挙をNative 24 checkと.NET 116 test（実`.aux2` handler相互運用を含む。実AviUtl2 E2EはWP11で実施）で検証済み
