...
- M1証拠: locked restore、C# 8 project build、native `.aux2` build、29 tool schema conformance、C#/C++ 40-byte header golden vector
...
V1の29 tools、5 resources、4 promptsを削減せず実装する。各末端taskは原則15～60分の検証可能な単位とし、失敗時に層を特定できる順で進める。
...
| W1.7 | Schema conformance testを作成 | 29 input/output schemaとC# DTOの代表値・拒否値を検証 | W1.2-W1.6 | 60分 |
...
| W2.6 | `CapabilityService`を実装 | 29操作、version、固定limit DTOを返す | W2.3 | 60分 |
...
| W10.3 | catalog parity testを実装 | 29/5/4の名前、schema、description、annotationを比較 | W10.1,W10.2 | 60分 |
...
| M7 V1 complete | WP9～WP11完了 | 29/5/4 parity、34 AC、package smoke、実機report |
...
| 観点 | 発見 | 反映 |
|---|---|---|
| 機能削減 | queryやPSDを後回しにしたままMVP完了扱いする危険 | M7だけをV1完了とし29/5/4 parityを必須化 |
...
- 29 tools、5 resources、4 promptsがmachine catalogと一致する。
