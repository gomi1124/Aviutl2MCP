...
2026-07-27時点で34件すべてが合格済み。保存・数値`0`更新の追加実機証跡はcommit `93227fc` のクリーンな作業ツリーで生成し、元fixtureのSHA-256不変とharness所有processだけのcleanupを確認した。
...
| AC-MCP-001 | `stdio.offline-initialize` | stdio black-box | AviUtl2なしでinitializeと29 tools listが成功 | [CI] `contract` |
| AC-MCP-002 | `pipe.late-connect` | fake bridge統合 | Server維持中にbridge起動しReadyへ遷移 | [CI] `integration` |
| AC-MCP-003 | `mcp.catalog-snapshot` | MCP contract | 29 tools、5 resources、4 promptsとSchema catalogが一致 | [CI] `contract` |
...
| AC-EDT-003 | `real.object-edit-lifecycle` | 専用AviUtl2実機 | move/name/item（数値`1→0`を含む）/state/deleteを再取得で確認 | `artifacts/real-e2e/019f9f2f-15bf-7b08-aaed-f5c390be81a3/debug-report.json` |
